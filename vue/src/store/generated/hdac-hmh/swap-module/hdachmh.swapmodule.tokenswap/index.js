...
import { SwapRequest } from "./module/types/tokenswap/SwapRequest";
    ...
    actions: {
        ...
        async QuerySwapRequest({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QuerySwapRequest', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QuerySwapRequestAll({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await (await initQueryClient(rootGetters)).querySwapRequestAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QuerySwapRequestAll', payload: { options: { all }, params: { ...key }, query } });
